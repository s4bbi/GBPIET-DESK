...
export default function AdminList() {
  ...
  const handleDelete = async () => {
    try {
      await api.delete(`/api/v1/admin/${confirmDeleteId}`);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await api.post("/api/v1/admin/register", formData);
          ...
          {admins.map((admin) => (
            <li key={admin._id} className="flex items-center justify-between border-b pb-2 text-gray-700">
              ...
              {isSuperAdmin && admin._id !== currentUser?.id && (
                <button onClick={() => setConfirmDeleteId(admin._id)} className="text-red-500 hover:text-red-600 transition">
                  <FiTrash2 size={18} />
                </button>
              )}
            </li>
